...
/*---------------------------------------------------------
Purpose: The purpose of this is to use 4 test cases in the 
client to test the methods of a linked list class. We test 
through adding deleting and inserting at different locations.
---------------------------------------------------------*/

/*---------------------------------------------------------
Algorithm: The algorithm we use is implemented in the linked
list. The four cases are divided into 4 functions. We use a 
switch to select each function displayed in the menu. The algorithm
uses the linked list data structure to perform basic insertion 
and deletion operations on the list
---------------------------------------------------------*/
void caseOne()
{
  cout << "CASE 1:------------------- " << endl;
  ...
  //1 check if the list is  empty and report the result  
  cout << "Step " << 1 << endl;
  if(L.isEmpty())
...
{ 
  ...
  cout << "Step " << c << endl; c++;
  try{
  L2.addbeforeIth(0,0);//Tries to add out of range 
  } 
  catch(llist::OutOfRange){
  cerr<<"ERROR: Out of range..."<<endl;//Catches Out of Range error
  }
  ...
  while(!L2.isEmpty())
    {
     L2.deleteRear(x);// Deletes Until List is Empty
    }
  cout<<endl;
...
void caseThree()
{
  ...
  try {
    ...
    cerr<<"ERROR: Underflow of Elements..."<<endl;
  }
}   //end of case 3
...
//Keep Running Menu until user enters a 0  
  do
    {
